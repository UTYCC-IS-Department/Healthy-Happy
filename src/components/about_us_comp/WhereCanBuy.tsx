...
export const WhereCanBuy: React.FC<WhereCanBuyProps> = ({
  retailers,
}) => {
  const [hoveredId, setHoveredId] = useState<number | null>(null);

  const locations: LocationItem[] = retailers.map((name, index) => ({
    id: index + 1,
    name,
  }));
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-4 sm:gap-x-6 gap-y-10 sm:gap-y-12 lg:gap-y-14">
          ...
          >
            ...
            <p className="text-sm font-semibold leading-5 text-gray-800">
              Apart from major modern trade venues, our products are widely
              available at prominent regional supermarkets and trusted local
              retail shops across the country
            </p>
          </motion.div>
